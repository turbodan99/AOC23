...
# Find all the numbers which are adjacent to a *
# log them and the coords of the star
gears = {}
            ...
            for number in numbers:
                ...
                for check in numbers[number]:
                    ...
                    for chk in check_these:
                        cx = chk[0]
                        cy = chk[1]
                        if ord(new_grid[cx,cy]) == ord('*') and done == False:
                            key = str(cx) + '_' + str(cy)
                            if key in gears:
                                gears[key] += number.split()
                            else:
                                value = number.split()
                                gears[key] = value
                            done = True
...
# Once finished, get the stars with two numbers against them, multiply together and add to total
total = 0
